...
module.exports = ({
  ...
  visit(markdownAST, `code`, node => {
    ...
    const highlightedCode = highlightCode(languageName, node.value, escapeEntities, highlightLines, noInlineHighlight);
    ...
    const useCommandLine = [`bash`].includes(languageName) && (prompt.global || outputLines && outputLines.length > 0 || promptUserLocal || promptHostLocal); // prettier-ignore

    node.value = `` + `<div class="${highlightClassName}" data-language="${languageName}">` + `<pre${numLinesStyle} class="${className}${numLinesClass}">` + `<code class="${className}">` + `${useCommandLine ? commandLine(node.value, outputLines, promptUser, promptHost) : ``}` + `${highlightedCode}` + `</code>` + `${numLinesNumber}` + `</pre>` + `</div>`;
  });
  ...
  if (!noInlineHighlight) {
    visit(markdownAST, `inlineCode`, node => {
      ...
      node.type = `html`;
      node.value = `<code class="${className}">${highlightCode(languageName, node.value, escapeEntities)}</code>`;
    });
  }
};